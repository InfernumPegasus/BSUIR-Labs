...
constexpr double PerformMatrixMultiplication(const Matrix<Size> &A, const Matrix<Size> &B,
    ...
    constexpr size_t SizeSquare = Size * Size;
    ...
    if (synchronous) {
        ...
        MPI_CHECK(MPI_Scatter(A.PlainArray(), SizeSquare / processors, MPI_INT, resultA[from],
        ...
        MPI_CHECK(MPI_Iscatter(A.PlainArray(), SizeSquare / processors, MPI_INT, resultA[from],
                               SizeSquare / processors, MPI_INT, 0, MPI_COMM_WORLD,
                               &request[1]));
    ...
    MPI_CHECK(MPI_Gather(C.PlainArray()[from], SizeSquare / processors, MPI_INT,
                         resultMatrix.PlainArray(), SizeSquare / processors, MPI_INT, 0,
                         MPI_COMM_WORLD));
