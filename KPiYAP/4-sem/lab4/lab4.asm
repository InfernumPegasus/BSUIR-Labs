...
.data
    ...
    score_color equ 00001000b      ;temno seriy (temniy grey)
...
ShowScore	proc 
	...
	nextdigit:
		...
		mov ah, 0Eh	; вывод символа в режиме телетайпа
		mov al, 0Dh	; символ возврат каретки
		int 10h
		mov	bl, score_color
	outdigit:
		pop	ax		
...
ClearKbd	proc 
	next:
		mov	ah, 01h	; проверка наличия символа в буфере
		int	16h		
		jz	short nokeys	
		xor	ah,ah
		int	16h
...
; set segment registers:
    ...
    ;ustanovka 16 cvetovogo video reshima
    mov ah, 00
    mov al, 03
    int 10h
    
    ;Otrisovka granic
    push 0B800h			; 0D800:0000 - 0D800:FFFF
    pop es 
    mov al, ' '
    mov ah, land_color
