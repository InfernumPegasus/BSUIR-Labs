...
fseekCurrent MACRO settingPos
	...
	mov ah, 42h			; ф-ция DOS уставноки указателя файла
	mov al, 1			; 1 - смещение указателя от текущей позиции
...
main:
    ...
    xor ax, ax
    mov al, [enteredString+1]	 ; Запоминаем длину введенной последовательности
...
findPoint:				; Ищем точку с конца файла, т.к. после неё идет расширение файла
    mov al, '.'
    mov bl, [di]
    cmp al, bl
    je pointFound
...
openFiles PROC
    ...
    lea dx, destinationPath	; asciz - строка ; cf == 1 -> ax
    int 21h
